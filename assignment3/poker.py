...
def mapper(record):
    ...
    for i in range(0, 10): 
       if (counts[straightrunfaces[i]] and 
          ...
          break
    if not is_straight:
        is_F4cardstraight = False    
        F4cardstraight = 'A23456789TJQKA';   # note: ace ('A') appears twice
        for i in range(0, 11): 
            if (counts[F4cardstraight[i]] and 
                 counts[F4cardstraight[i+1]] and 
                 counts[F4cardstraight[i+2]] and 
                 counts[F4cardstraight[i+3]]): 
              is_F4cardstraight = True 
              break
    ...
    elif is_F4cardstraight: 
         mr.emit_intermediate('F4cardstraight', '1')
    elif is_trip: 
